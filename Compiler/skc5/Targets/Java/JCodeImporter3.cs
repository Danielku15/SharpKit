...
namespace SharpKit.Compiler.Java
{
    partial class JCodeImporter : ICSharpResolveResultVisitor<JNode>
    {
        ...
        static string GetJArrayType(IType arrayType)
        {
        ...
        {
            var typeDef = res.TargetType.GetDefinitionOrArrayType();
                ...
                if (info.ConversionTargetType != null && !UseNativeFunctions(info.ConversionTargetType))//delegate type
                {
                    ...
                    if (parentMethod == null || !JMeta.ForceDelegatesAsNativeFunctions(parentMethod))
                    {
                        ...
                        if (instanceContext != null)
                            node3 = CreateJDelegate(instanceContext, func);
                        else if (firstPrm != null)
                            node3 = CreateJExtensionDelegate(firstPrm, func);
                    }
                }
            }
            return node3;
        ...
        public JNode VisitOperatorResolveResult(OperatorResolveResult res)
        {
            ...
            else if (res.Operands.Count == 2)
            {
                ...
                {
                    var att = JMeta.GetJExportAttribute(Project.Compilation.MainAssembly);
        ...
        public JNode VisitLambdaResolveResult(LambdaResolveResult res)
        {
            ...
            //return Visit(conv);
            var func = new JFunction { Parameters = res.Parameters.Select(t => JIdentifier(t.Name)).ToList() };
            var body = res.Body;
            JNode body2;
            var info = res.GetInfo();
        ...
        public static string JIdentifier(string name)
        {
            return name.Replace("<", "$").Replace(">", "$");
        }
        public JNode VisitLocalResolveResult(LocalResolveResult res)
        {
            var node2 = J.Member(JIdentifier(res.Variable.Name));
        ...
        private JNode VisitConversion(ResolveResult input, Conversion conversion, IType conversionType)
        {
            ...
            if (conversion.IsMethodGroupConversion)
            {
                ...
                var exp = J.JAccess(me).Invoke(me.Parameters.Select(t=>J.Member(t.Name)).ToArray());
                var st = J.Return(exp);
            ...
            {
                var typeDef = conversionType.GetDefinitionOrArrayType();
        ...
        private JNode VisitInvocationResolveResult(CSharpInvocationResolveResult res)
        {
            ...
            var jMember = JNaming.JAccess(member);
            if (res.TargetResult != null && !member.IsStatic() && member.SymbolKind != SymbolKind.Constructor) //TargetResult==null when ctor
            {
                ...
                if (jMember.PreviousMember != null)
                    throw new NotSupportedException();
                jMember.PreviousMember = target;
            }
            ...
            {
                JMemberExpression jmex = node2.Arguments[i] as JMemberExpression;
                if (jmex != null)
                    node2.Arguments[i] = jmex.PreviousMember;//remove the .Value ref wrapper
        ...
        private JNode VisitInvocationResolveResultAsCtor(CSharpInvocationResolveResult res)
        {
            ...
            var att = ce == null ? null : ce.GetJTypeAttribute();
            if (att != null && att.Mode == JMode.Json && (meAtt == null || meAtt.Name == null))
            {
        ...
        #region IResolveResultVisitor<JNode> Members
        ...
        #region ICSharpResolveResultVisitor<JNode> Members
        ...
        JExpression GetNonStaticMethodContextJ(ResolveResult res)
        {
