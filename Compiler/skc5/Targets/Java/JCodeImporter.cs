    ...
    partial class JCodeImporter 
    {
        ...
        void TransformIntoBaseMethodCallIfNeeded(CSharpInvocationResolveResult res, JInvocationExpression node2)
        {
            ...
            if (target != null && target.CausesNonVirtualInvocation) //base.
            {
                ...
                if (ce != null && JMeta.IsExtJType(ce.GetDefinitionOrArrayType()))
                {
        ...
        JExpression CreateJDelegate(JExpression instanceContext, JExpression func)
        {
        ...
        JExpression CreateAnonymousJDelegate(JExpression instanceContext, JExpression func)
        {
            if (instanceContext == null)
                return func;
        ...
        JExpression CreateJExtensionDelegate(JExpression prm1, JExpression func)
        {
            if (prm1 == null)
                return func;
        ...
        JNode CreateJDelegateIfNeeded(JFunction func, IMember currentMember, IType delType, bool isAnonymous)
        {
            if (currentMember != null && !currentMember.IsStatic && !UseNativeFunctions(delType) && !JMeta.ForceDelegatesAsNativeFunctions(currentMember))
            {
                ...
                JExpression wrapper;
                if (isAnonymous)
                    wrapper = CreateAnonymousJDelegate(instanceContext, func);
                else
                    wrapper = CreateJDelegate(instanceContext, func);
                return wrapper;
            }
            else
            {
        ...
        bool UseNativeFunctions(ITypeDefinition delegateType)
        {
            ...
            {
                var att2 = delegateType.GetMetadata<JDelegateAttribute>();
        ...
        public JNode Visit(ResolveResult res)
        {
            try
            {
                if (CompilerConfiguration.Current.EnableLogging)
                {
                    ...
                    Log.WriteLine("JCodeImporter: Visit ResolveResult: {0}, AstNode: {1}", res, ToDebug(node3));
                }
                if (BeforeConvertCsToJResolveResult != null)
                    BeforeConvertCsToJResolveResult(res);
                ...
                //        ex.AstNode = node;
                //}
                if (AfterConvertCsToJResolveResult != null)
                    AfterConvertCsToJResolveResult(res, node2);
                return node2;
            }
            catch (CompilerException e)
            {
