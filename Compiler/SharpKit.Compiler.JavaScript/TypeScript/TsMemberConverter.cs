...
namespace SharpKit.Compiler.JavaScript.TypeScript
{
    class TsMemberConverter
    {
        ...
        private TsTypeRef Visit(IType type)
        {
            ...
            if (typeDef!=null)
            {
                ...
                if (Utils.Sk.IsJsonMode(typeDef))
                    return new TsTypeRef { Name = "Object" };
                if (type.Kind == TypeKind.Delegate && !Utils.Sk.IsJsExported(type.GetDefinition()))
                {
