...
namespace SharpKit.Compiler.JavaScript.Conversion
{
    class JsMemberConverter : IMemberConverter
    {
        ...
        protected IEnumerable<IField> GeneratePropertyFields(ITypeDefinition ce, bool isStatic)
        {
            ...
            {
                if (!Utils.Sk.IsJsExported(pe))
                    continue;
                if (Utils.Sk.IsNativeField(pe))
        ...
        protected JsBlock ExportConstructorBody(IMethod ctor)
        {
            ...
            var isClr = Utils.Sk.IsClrType(ce);
            var isPrototype = Utils.Sk.IsNativeType(ce);
            var statements = new List<JsStatement>();
            //instance fields initializations
            if (!Utils.Sk.InlineFields(ce))
            {
                var isGlobal = Utils.Sk.IsGlobalType(ce);
            ...
            if (!ctor.IsStatic)
            {
                ...
                if (invocation != null)
                {
                    ...
                    var isBaseClr = Utils.Sk.IsClrType(baseThisCe.GetDefinition());
                    var isBasePrototype = Utils.Sk.IsNativeType(baseThisCe.GetDefinition()) && !Utils.Sk.IsJsonMode(baseThisCe.GetDefinition()) && !Utils.Sk.IsGlobalType(baseThisCe.GetDefinition());//happens when prototype inherits from json
        ...
        void ExportConstructorParameters(IMethod ctor, JsFunction func)
        {
            ...
            if (!Utils.Sk.IgnoreTypeArguments(ce))
            {
                //danel
        ...
        {
            var jma = Utils.Sk.GetJsMethodAttribute(me);
        ...
        protected JsBlock ExportMethodBody(IMethod me)
        {
            ...
            var nativeCode = Utils.Sk.GetNativeCode(me);
            if (nativeCode != null)
            {
            ...
            if (def == null || def.IsNull)
            {
                ...
                else if (me.IsAutomaticPropertyAccessor())
                {
                    ...
                    else if (!Utils.Sk.IsGlobalMethod(me))
        ...
        {
            var list = new List<string>();
            if (!Utils.Sk.IgnoreGenericMethodArguments(me) && me.GetGenericArguments().Count() > 0)
            {
        ...
        {
            return !ev.IsExplicitInterfaceImplementation && Utils.Sk.IsJsExported(ev);
        }
        ...
        protected bool ShouldExportMethod(IMethod mde)
        {
            ...
            if (Utils.Sk.NewInterfaceImplementation) ret = true;

            if (!Utils.Sk.NewInterfaceImplementation)
            {
        ...
        protected bool ShouldExportConstructor(IMethod ctor)
        {
            ...
            if (att != null && !att.Export)
                return false;
            if (ctor.IsGenerated(Compiler.Project) && Utils.Sk.OmitDefaultConstructor(ctor.GetDeclaringTypeDefinition()))
                return false;
            return true;
        }
        ...
        {
            return (Utils.Sk.IsJsExported(me) && me.GetMethodDeclaration() != null && !me.IsAnonymousMethod());
        }
        ...
        public List<IMember> GetMembersToExport(ITypeDefinition ce)
        {
            ...
            if (ctor == null && !Utils.Sk.OmitDefaultConstructor(ce))
            {
            ...
            var inlineFields = Utils.Sk.InlineFields(ce);
            if (!inlineFields)
            {
        ...
        protected virtual IType GetBaseClassIfValid(ITypeDefinition ce, bool recursive)
        {
            ...
            {
                if (Utils.Sk.IsClrType(baseClass.GetDefinition()) || (Utils.Sk.IsNativeType(baseClass.GetDefinition()) && !Utils.Sk.IsGlobalType(baseClass.GetDefinition())) || !recursive)
                    return baseClass;
        ...
        InvocationResolveResult GetConstructorBaseOrThisInvocation2(IMethod ctor)
        {
            ...
            {
                var ce = ctor.GetDeclaringTypeDefinition();
                if (Utils.Sk.OmitInheritance(ce))
                    return null;
                var baseType = GetBaseClassIfValid(ce, true);
                if (baseType != null)
                {
        ...
        protected List<IMethod> GetAccessorsToExport(IProperty pe)
        {
            ...
            if (pe.IsAutomaticProperty() && !Utils.Sk.IsNativeField(pe))
            {
                list.Add(pe.Getter);
                list.Add(pe.Setter);
            }
            ...
            list.RemoveAll(t => !Utils.Sk.IsJsExported(t));
            return list;
        }
        ...
        JsMode GetJsMode(ITypeDefinition ce)
        {
            ...
            var isNative = Utils.Sk.IsNativeType(ce);
            if (isNative)
        ...
        {
            bool valuesAsNames;
            Utils.Sk.UseJsonEnums(ce, out valuesAsNames);
            ...
            constants.RemoveAll(t => !Utils.Sk.IsJsExported(t));
