...
namespace SharpKit.Compiler.JavaScript.Conversion
{
    class JsMemberConverterClr : JsMemberConverter
    {
        ...
        public override JsNode _Visit(IProperty pe)
        {
            ...
            {
                if (Utils.Sk.InlineFields(pe.GetDeclaringTypeDefinition()))
                {
        ...
        {
            //HACK: backward
            if (jsType.Kind == JsClrTypeKind.Interface && CssCompressorExtensions.IsNullOrEmpty(jsType.baseTypeName))
        ...
        private JsNode ExportClassStructInterface(ITypeDefinition ce)
        {
            ...
            if (!ce.IsInterface() || Utils.Sk.NewInterfaceImplementation)
            {
            ...
            {
                {
                    ...
                    if (!Utils.Sk.IsJsExported(attType))
                        return false;
                }
                return true;
            }).ToList();
