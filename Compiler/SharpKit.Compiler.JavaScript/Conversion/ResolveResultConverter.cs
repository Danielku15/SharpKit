...
namespace SharpKit.Compiler.JavaScript.Conversion
{
    class ResolveResultConverter : ICSharpResolveResultVisitor<JsNode>
    {
        ...
        {
            if (Utils.Sk.OmitCasts(res.TargetType))
            {
                return Js.True();
                //var node2 = Visit(res.Input);
                //return node2;
            }
            else if (Utils.Sk.NativeCasts(res.TargetType))
            {
                var typeFieldName = Utils.Sk.GetJsonTypeFieldName(res.TargetType.GetDefinitionOrArrayType());
                if (typeFieldName != null && Utils.Sk.IsJsonMode(res.TargetType.GetDefinitionOrArrayType()))
                {
        ...
        public JsNode VisitMethodGroupResolveResult(MethodGroupResolveResult res)
        {
            ...
            if (info != null && (instanceContext != null || firstPrm != null))
            {
                ...
                if (info.ConversionTargetType != null && !UseNativeFunctions(info.ConversionTargetType))//delegate type
                {
                    ...
                    if (parentMethod == null || !Utils.Sk.ForceDelegatesAsNativeFunctions(parentMethod))
                    {
        ...
        private JsNode VisitConversion(ResolveResult input, ICSharpCode.NRefactory.Semantics.Conversion conversion, IType conversionType)
        {
            ...
            else if (conversion.IsUserDefined)
            {
                ...
                var nativeOverloads = Utils.Sk.UseNativeOperatorOverloads(typeDef);
                if (nativeOverloads)
                    return Visit(input);
            ...
            else if (conversion.IsTryCast || conversion.IsExplicit)
            {
                ...
                var omitCasts = Utils.Sk.OmitCasts(conversionType);
                if (omitCasts)
                    return Visit(input);
                if (Utils.Sk.NativeCasts(conversionType))
                {
        ...
        public JsNode VisitCSharpInvocationResolveResult(CSharpInvocationResolveResult res)
        {
            if (res.Member.IsConstructor())
            {
                if (res.Type is AnonymousType)
                {
                    ...
                    if (parentType != null && !Utils.Sk.UseNativeJsons(parentType))
                    {
        ...
        public JsNode VisitMemberResolveResult(MemberResolveResult res)
        {
            ...
            else if (Utils.Sk.IsEntityFunctionProperty(res.Member, res))//(Entity)node.entity))
            {
                var pe = (IProperty)me;
            ...
            else if (me.IsEnumMember() && Utils.Sk.UseJsonEnums(me, out enumValuesAsNames))
            {
                var me2 = (IField)me;
        ...
        internal JsExpression InitializersToJson(IList<ResolveResult> initializerStatements, IType type)
        {
            ...
            //FIX for issue 325:
            if (Utils.Sk.IsNativeArrayEnumerator(type.GetDefinition()) && (initializerStatements.IsEmpty() || initializerStatements[0] is CSharpInvocationResolveResult))
            {
        ...
        {
            if (currentMember != null && !currentMember.IsStatic && !UseNativeFunctions(delType) && !Utils.Sk.ForceDelegatesAsNativeFunctions(currentMember))
            {
        ...
        bool IsNonStatic(IEntity me)
        {
            if (!me.IsStatic())
                return true;
            if (me is IMethod && Utils.Sk.ExtensionImplementedInInstance((IMethod)me))
                return true;
            return false;
        }
