...
using SharpKit.Compiler.JavaScript.TypeScript;
...
namespace SharpKit.Compiler.JavaScript.Conversion
{
    class JsTypeConverter : ITypeConverter
    {
        ...
        {
            return Utils.Sk.IsJsExported(ce);
        }
        ...
        public void Process()
        {
            ...
            var byFile = allTypesToExport.GroupBy(ce => Compiler.PathMerger.ConvertRelativePath(Utils.Sk.GetExportPath(ce))).ToDictionary();
            ...
            if (Utils.Sk.ExportTsHeaders(Compiler.Project.Compilation))
            {
        ...
        {
            var list2 = list.Where(t => Utils.Sk.IsNativeType(t) && t.GetBaseTypeDefinition() != null && Utils.Sk.IsNativeType(t.GetBaseTypeDefinition())).ToList();
            foreach (var ce in list2)
            {
                var ce3 = ce;
                while (true)
        ...
        JsMemberConverter GetMemberConverter(ITypeDefinition ce)
        {
            ...
            var isExtJs = Utils.Sk.IsExtJsType(ce);
            var isGlobal = Utils.Sk.IsGlobalType(ce) && !isExtJs;
            var isNative = Utils.Sk.IsNativeType(ce) && !isExtJs;
            if (isGlobal)
            {
